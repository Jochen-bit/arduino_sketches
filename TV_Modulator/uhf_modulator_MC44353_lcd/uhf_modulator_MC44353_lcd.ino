...
void mod_start(byte channel, byte soundIdx, boolean testMode) {
  ...
  Wire.write(0x88);
  Serial.println("Write to i2C:");
  Serial.println (0x88);
  Wire.write(0x40 | mod_sound[soundIdx]);
  Serial.println (0x40 | mod_sound[soundIdx]);
  Wire.write(fm);
  Serial.println (fm);
  Wire.write(fl);
  Serial.println (fl);
...
void mod_changeChannel(byte channel, boolean testMode) {
  ...
  mod_setFreqBytes(fm, fl, channel, testMode);
  Serial.println("Write to i2C(change chanal):");
  Wire.beginTransmission(MOD_ADDRESS);
  Wire.write(fm);
  Serial.print ("fm: ");
  Serial.println (fm);
...
void mod_setSoundCarrier(byte idx) {
  ...
  Wire.write(0x88);
  Serial.print ("C0: ");
  Serial.println (0x40 | mod_sound[idx]);
